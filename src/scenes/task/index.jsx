...
const backgroundImages = ["/assets/bg1.jpg", "/assets/bg2.jpg", "/assets/bg3.jpg"];
...
const DocumentManagement = ({ documents, onUpload, isUploading, boardId }) => {
  ...
  const confirmDeleteDocument = async () => {
    ...
    try {
      ...
      await updateDoc(boardRef, { documents: arrayRemove(documentToDelete) });
  ...
  const getFileIcon = (fileName) => {
    ...
    if (["xlsx", "xls", "csv"].includes(extension)) return <ExcelIcon color="success" />;
  ...
  return (
    <Box sx={{ mt: 2, width: "100%" }}>
          ...
          <List dense>
            {documents.map((doc, index) => (
              <ListItem
                ...
                sx={{ "&:hover": { bgcolor: alpha(theme.palette.action.hover, 0.5), borderRadius: 1 } }}
              >
                <ListItemIcon>{getFileIcon(doc.name)}</ListItemIcon>
                <ListItemText
                  primary={
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      <Typography sx={{ color: "text.primary", fontWeight: 500 }}>{doc.name}</Typography>
                      <Typography
                        variant="caption"
                        sx={{
      ...
      <Dialog open={deleteDialogOpen} onClose={() => !isDeleting && setDeleteDialogOpen(false)} PaperProps={{ sx: { borderRadius: 3, bgcolor: "background.paper" } }}>
        <DialogTitle sx={{ pb: 1 }}>
        ...
        <DialogActions sx={{ p: 3 }}>
          <Button onClick={() => setDeleteDialogOpen(false)} disabled={isDeleting} sx={{ color: "text.secondary", textTransform: "none" }}>
            Cancel
          </Button>
          <Button
